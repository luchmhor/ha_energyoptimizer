...
║                                                                            ║
║  Changelog:                                                                ║
║   2026.06.08  3-strategy model (FOLLOW_GRID/HOLD/GRID_CHARGE); data-driven  ║
║               horizon; pure cost LP; PV cost 4.5ct; terminal value = PV    ║
║               floor (fixes peak-charging); battery throughput cost;        ║
║               PV-surplus → FOLLOW_GRID (fixes high-price morning import);  ║
║               executed-history outlook + Why column + per-day archive.     ║
║   2026.06.10  LP rebuilt on 4 variable groups: PV-surplus charging is now  ║
║               an LP DECISION (y_pv) instead of a fixed cum_pv term — fixes ║
║               PV surplus being double-counted in the SOC bounds (the LP    ║
║               could plan more late discharge than the battery would hold). ║
║               Grid charging split off as y_g, with a linear SOC-ceiling    ║
║               surrogate (GRID_CHARGE_SOC_CEILING_PCT is now enforced, not  ║
║               just documented). Date-aware fallback prices (an EPEX outage ║
║               no longer collapses the horizon to 1 h). Outlook + history   ║
║               simulate HOLD as the hardware executes it (full-load import, ║
║               ALL PV→battery) instead of "PV offsets load". Per-slot       ║
║               anti-curtailment exception via a passive-SOC envelope.       ║
║               Weekday-correct consumption profile, cached per day, with a  ║
║               configurable quantile. Outlook keeps the last good table     ║
║               under a STALE banner instead of wiping it on a bad cycle.    ║
║               SOC emergency is edge-triggered with hysteresis and replans  ║
║               automatically on recovery. All file I/O moved off the event  ║
║               loop (task.executor + @pyscript_compile). Influx forecast    ║
║               SCHEMA CHANGE: `strategy` and `minutes_ahead` are FIELDS now ║
║               and the only tag is `phase` — update Grafana queries. Fixed  ║
║               the mode-ID table in the config comment below (it documented ║
║               an old, contradictory 4-mode scheme). Removed unused         ║
║               OPPORTUNITY_COST_WEIGHT / GRID_CHARGE_SOC_CHEAP_PCT.         ║
║   2026.06.10b Publishes sensor.energy_optimizer_forecast every cycle: the  ║
║               combined executed-history + forecast slot list as a `data`   ║
║               attribute (time/strategy/cons_w/pv_w/grid_w/batt_w/price_ct/ ║
║               soc_pct/forecast-flag), for dashboard cards (plotly-graph    ║
